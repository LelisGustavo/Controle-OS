            ...
            <section class="content">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> <?= isset($dados) ? 'Alterar o equipamento cadastrado' : 'Aqui você podera cadastrar seus novos equipamentos' ?> </h3>
                    </div>
                    <div class="card-body">
                        <form id="form_cad" action="equipamento.php" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 obg" style="width: 100%;" name="tipo_equipamento"
                                    ...
                                    <?php foreach ($tipos as $item) { ?>
                                        <option value="<?= $item['id'] ?>"  <?= isset($dados) ? ($item['id'] == $dados['tipo_id'] ? 'selected' : '') : '' ?>  ><?= $item['nome'] ?></option>
                                        <?php } ?>
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control obg" placeholder="Digite aqui..." name="identificacao"
                                    id="identificacao" value="<?= isset($dados['identificacao']) ? $dados['identificacao'] : '' ?>">
                            </div>
                            <div class="form-group">
                                <label>Descrição</label>
                                <textarea class="form-control obg" rows="3" placeholder="Digite aqui..."
                                    name="descricao" id="descricao"><?= isset($dados['descricao']) ? $dados['descricao'] : '' ?></textarea>
                            </div>
                            <button class="btn btn-outline-success" name="btn_gravar" 
                                onclick="return GravarEquipamento('form_cad')"><?= $acao ?></button>
                    </div>
                    </form>
                    <!-- /.card-body -->
                </div>
